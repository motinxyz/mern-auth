...
export const verifyUserEmail = async (token) => {
  ...
  authServiceLogger.debug({ key: verifyKey }, "Constructed Redis key for verification token.");

  // 2. Check if the token exists in Redis
  const userDataJSON = await redisClient.get(verifyKey);
  if (!userDataJSON) {
    authServiceLogger.warn({ key: verifyKey }, "Verification token not found in Redis.");
  ...
  if (!user) {
    ...
    authServiceLogger.error({ userId: userData.userId }, "User from token not found in database.");
