...
export const errorHandlerFactory = ({ logger, t }) => {
  ...
  function convertExternalError(err, req) {
    ...
    if (err.name === "ValidationError" && err.errors) {
      ...
      return new ValidationError(errors, req.t || t); // Use req.t if available, otherwise injected t
    }
  ...
  return (err, req, res, next) => {
    ...
    const response = {
      ...
      message: (req.t || t)(apiError.message), // Translate the main message
      errors: Array.isArray(apiError.errors)
        ? apiError.errors.map((e) => {
            const errorObject = {
              field: e.field,
              message: (req.t || t)(e.message, e.context), // Translate detailed error messages
