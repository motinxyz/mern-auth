...
const envSchema = z.object({
  ...
  MONGO_URI: z.string().min(1, { message: JSON.stringify({ message: "validation:required", params: { field: "MONGO_URI" } }) }).refine((val) => val.startsWith("mongodb://") || val.startsWith("mongodb+srv://"), { message: JSON.stringify({ message: "validation:invalidUrl", params: { field: "MONGO_URI" } }) }),
  DB_NAME: z.string().min(1).default(DEFAULTS.DB_NAME),
  CLIENT_URL: z.string().default(DEFAULTS.CLIENT_URL).refine((val) => urlRegex.test(val), { message: JSON.stringify({ message: "validation:invalidUrl", params: { field: "CLIENT_URL" } }) }),
  REDIS_URL: z.string().refine((val) => urlRegex.test(val), { message: JSON.stringify({ message: "validation:invalidUrl", params: { field: "REDIS_URL" } }) }),
  ...
  EMAIL_FROM: z.string().email({ message: JSON.stringify({ message: "validation:invalidEmail", params: { field: "EMAIL_FROM" } }) }).optional(),
  VERIFICATION_TOKEN_EXPIRES_IN: z.coerce.number().default(DEFAULTS.VERIFICATION_TOKEN_EXPIRES_IN),
