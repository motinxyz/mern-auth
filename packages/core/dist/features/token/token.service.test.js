...
describe("Token Service", () => {
    ...
    describe("createVerificationToken", () => {
        it("should create a token, hash it, and store it in Redis", async () => {
            ...
            const storedValue = await mockRedis.get(`${mockConfig.redis.prefixes.verifyEmail}${hashedToken}`);
            expect(storedValue).not.toBeNull();
            expect(JSON.parse(storedValue)).toEqual({
                userId: user.id,
                email: user.email,
            });
            const ttl = await mockRedis.ttl(`${mockConfig.redis.prefixes.verifyEmail}${hashedToken}`);
            expect(Math.abs(ttl - mockConfig.verificationTokenExpiresIn)).toBeLessThanOrEqual(1);
        ...
        it("should throw TokenCreationError if redis fails", async () => {
            ...
            await expect(tokenService.createVerificationToken(user)).rejects.toThrow(TokenCreationError);
            expect(mockError).toHaveBeenCalledWith({ err: expect.any(Error) }, "Token creation failed");
        });
    });
});
