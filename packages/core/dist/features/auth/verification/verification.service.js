...
export class VerificationService {
    ...
    async verify(token) {
        const hashedToken = crypto
            ...
            .update(token)
            .digest("hex");
        const verifyKey = createVerifyEmailKey(this.config.redis.prefixes.verifyEmail, hashedToken);
        ...
        this.logger.debug({ key: verifyKey, data: userDataJSON }, VERIFICATION_MESSAGES.TOKEN_FOUND_REDIS);
        // Parse user data
        let userData;
        try {
        ...
        if (!user) {
            this.logger.error({ userId: userData.userId }, VERIFICATION_MESSAGES.USER_FROM_TOKEN_NOT_FOUND);
