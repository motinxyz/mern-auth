    ...
    async register(registerUserDto) {
        ...
        return withSpan("registration-service.register-user", async (rootSpan) => {
            ...
            await withSpan("registration-service.generate-token", async () => {
                this.logger.info({ userId: newUser._id }, REGISTRATION_MESSAGES.ORCHESTRATING_VERIFICATION);
                ...
                await withSpan("registration-service.queue-email", async () => {
                    try {
                        ...
                        await this.emailProducer.addJob(EMAIL_JOB_TYPES.SEND_VERIFICATION_EMAIL, {
                            user: {
                        ...
                        this.logger.error({ emailJobError, userId: newUser._id }, REGISTRATION_MESSAGES.ADD_EMAIL_JOB_FAILED);
            ...
            try {
                await this.redis.set(rateLimitKey, REDIS_RATE_LIMIT_VALUE, "EX", RATE_LIMIT_DURATIONS.VERIFY_EMAIL);
            }
            catch (redisSetError) {
                this.logger.error({ redisSetError, email }, REGISTRATION_MESSAGES.SET_RATE_LIMIT_FAILED);
