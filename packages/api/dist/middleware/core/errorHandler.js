...
function convertExternalError(err) {
    ...
    if (err.name === "ValidationError" && err.errors !== undefined) {
        ...
        const errors = Object.values(mongoErr.errors).map((e) => {
            ...
            if (contextRecord.minlength !== undefined)
                contextRecord.count = contextRecord.minlength;
            if (contextRecord.maxlength !== undefined)
                contextRecord.count = contextRecord.maxlength;
            return {
                field: e.path,
                message: e.message,
...
export const errorHandler = (err, req, res, _next) => {
    ...
    const response = {
        ...
        message: req.t(apiError.message, {
            count: apiError.errors !== undefined &&
