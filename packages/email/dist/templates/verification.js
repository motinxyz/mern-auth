 ...
 */
export const sendVerificationEmail = async (emailService, user, token, t, config, logger) => {
    const verificationLogger = logger.child({ module: "email-verification" });
    verificationLogger.debug({ userId: user.id, email: user.email }, t("email:logs.preparingVerification"));
    ...
    verificationLogger.debug({ userId: user.id }, t("email:logs.sendingVerification"));
