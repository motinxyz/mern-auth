 ...
 */
export async function handleBounce(emailLogRepository, userRepository, logger, t, bounceData) {
    const bounceLogger = logger.child({ module: "bounce-handler" });
    const { email, messageId, bounceType, bounceReason, timestamp } = bounceData;
    // Update email log
    const emailLog = await emailLogRepository.findOneAndUpdate({ messageId }, {
        status: "bounced",
    ...
    if (bounceType === "hard") {
        const wasSentViaResend = emailLog.provider === "resend-api" || !emailLog.provider;
        if (wasSentViaResend) {
            bounceLogger.info({ email, messageId, bounceReason }, EMAIL_MESSAGES.BOUNCE_HARD_RETRY_SMTP.replace("SMTP", "Alternate Provider"));
            return {
                success: true,
        ...
        const user = await userRepository.findOneAndUpdate({ email }, {
            emailValid: false,
            emailBounceCount: { $inc: 1 },
            lastEmailBounce: timestamp || new Date(),
        });
        if (user) {
            bounceLogger.warn({ email, userId: user._id, bounceReason }, EMAIL_MESSAGES.BOUNCE_USER_MARKED_INVALID);
        }
        return {
            success: true,
    ...
    if (bounceType === "soft") {
        bounceLogger.info({ email, messageId, bounceReason }, EMAIL_MESSAGES.BOUNCE_SOFT_RECORDED);
    ...
    if (bounceType === "complaint") {
        ...
        if (user) {
            bounceLogger.warn({ email, userId: user._id }, EMAIL_MESSAGES.BOUNCE_SPAM_COMPLAINT);
        }
        return {
            success: true,
            action: "unsubscribed",
...
//# sourceMappingURL=bounce-handler.js.map