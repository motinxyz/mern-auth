import { EmailDispatchError, createCircuitBreaker, CircuitBreakerError, ConfigurationError, withSpan, addSpanAttributes, hashSensitiveData, } from "@auth/utils";
import { i18nInstance } from "@auth/config";
import { emailSendTotal, emailSendDuration, emailCircuitBreakerState, } from "@auth/config";
...
class EmailService {
    ...
    constructor(options = {}) {
        if (!options.config) {
            throw new ConfigurationError(EMAIL_ERRORS.MISSING_CONFIG.replace("{config}", "config"));
        }
        if (!options.logger) {
            throw new ConfigurationError(EMAIL_ERRORS.MISSING_CONFIG.replace("{config}", "logger"));
        }
        if (!options.emailLogRepository) {
            throw new ConfigurationError(EMAIL_ERRORS.MISSING_CONFIG.replace("{config}", "emailLogRepository"));
        }
        if (!options.providerService) {
            throw new ConfigurationError(EMAIL_ERRORS.MISSING_CONFIG.replace("{config}", "providerService"));
        }
        ...
        this.emailBreaker.on("open", () => {
            ...
            this.logger.warn({
                ...
                stats: {
                    ...
                    failureRate: this.circuitBreakerStats.totalFires > 0
        ...
        this.emailBreaker.on("close", () => {
            ...
            this.logger.info({
                ...
                stats: {
                    ...
                    totalFailures: this.circuitBreakerStats.totalFailures,
                    successRate: this.circuitBreakerStats.totalFires > 0
     ...
     */
    async sendEmail({ to, subject, html, text, userId, type = "notification", metadata = {}, options = {}, // New options argument (e.g. preferredProvider)
     }) {
        return withSpan("email-service.send-email", async (span) => {
            ...
            try {
                ...
                this.logger.error({ err: logError, to, subject }, EMAIL_ERRORS.LOG_CREATION_FAILED);
            ...
            try {
                ...
                if (emailLog) {
                    try {
                        ...
                        this.logger.error({ err: updateError, emailLogId: emailLog._id }, EMAIL_ERRORS.LOG_UPDATE_FAILED);
                    }
                }
                this.logger.info({
                    messageId: info.messageId,
    ...
    async sendVerificationEmail(user, token, locale = "en", options = {}) {
        return withSpan("email-service.send-verification", async (span) => {
            // Add span attributes
            addSpanAttributes({
            ...
            verificationLogger.debug({ userId: user.id, email: user.email }, EMAIL_MESSAGES.PREPARING_VERIFICATION);
            ...
            verificationLogger.debug({ userId: user.id }, EMAIL_MESSAGES.SENDING_VERIFICATION);
