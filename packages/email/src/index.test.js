...
describe('Email Service', () => {
  ...
  it('should send an email successfully', async () => {
    mockBreakerFire.mockResolvedValue({ messageId: 'mock-message-id' }); // Mock fire method
  ...
  it('should send a verification email', async () => {
    const user = { id: 'user123', email: 'user@example.com', name: 'Test User' };
    const token = 'mockVerificationToken';
    const mockT = vi.fn((key, options) => {
      if (key === 'email:verification.subject') return 'Verify Your Email';
      if (key === 'email:verification.text') return `Hello ${options.name}, please verify your email: ${options.verificationUrl}`;
      return key;
    });
    ...
    expect(mockT).toHaveBeenCalledWith('email:verification.text', expect.any(Object));
  });
  ...
  it('should transition circuit breaker through open, half-open, and close states', async () => {
    ...
    const { logger } = await import('@auth/config');
    const emailUtilLoggerMock = logger.child.mock.results[0].value;
    expect(emailUtilLoggerMock.warn).toHaveBeenCalledWith(systemT('email:logs.circuitBreakerOpen'));
    ...
    expect(emailUtilLoggerMock.warn).toHaveBeenCalledWith(systemT('email:logs.circuitBreakerHalfOpen'));

    // 3. Mock success to close the circuit
    mockBreakerFire.mockResolvedValue({ messageId: 'mock-message-id-2' });
    await sendEmail(mailOptions);
    // Manually invoke the 'close' callback
    if (closeCallback) closeCallback();
    expect(emailUtilLoggerMock.info).toHaveBeenCalledWith(systemT('email:logs.circuitBreakerClosed'));
