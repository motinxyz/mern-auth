...
import { ConfigurationError, QueueError, ValidationError, CircuitBreakerError, createCircuitBreaker, withSpan, addSpanAttributes, } from "@auth/utils";
    ...
    async initialize() {
        return withSpan("QueueProducerService.initialize", async () => {
            ...
            if (this.enableCircuitBreaker) {
                this.circuitBreaker = createCircuitBreaker(async (jobName, data, jobOptions) => {
        ...
        return withSpan("QueueProducerService.addJob", async () => {
            ...
            if (this.jobSchema) {
                try {
                    ...
                    this.logger.error({ err: error, jobData: data }, QUEUE_ERRORS.JOB_DATA_VALIDATION_FAILED);
                    throw new ValidationError(error.errors, "Invalid job data");
                }
            }
            this.logger.info({ job: { name: jobName, data } }, QUEUE_MESSAGES.ADDING_JOB);
            ...
            try {
                ...
                if (customOptions.jobId) {
                    this.logger.debug({ jobId: customOptions.jobId, jobName }, QUEUE_MESSAGES.DEDUPLICATION_JOB);
                }
                return job;
            }
            catch (error) {
                ...
                this.logger.error({ err: error, jobData: data }, QUEUE_ERRORS.JOB_CREATION_FAILED);
                throw error;
            }
        });
    }
