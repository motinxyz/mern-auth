...
class ProducerService {
    ...
    async addJob(jobType, data, customOptions = {}) {
        return withSpan("ProducerService.addJob", async () => {
            ...
            try {
                const job = await this.queueService.addJob(jobType, { type: jobType, data }, customOptions);
                addSpanAttributes({ "job.id": job.id });
                this.logger.debug({ jobId: job.id, jobType }, QUEUE_MESSAGES.JOB_ADDED);
                return job;
            }
            catch (error) {
                this.logger.error({ err: error, jobType, data }, QUEUE_ERRORS.JOB_CREATION_FAILED);
                throw error;
            }
        });
    }
     ...
     * @param {object} customOptions - Optional BullMQ job options
     */
    async addJobWithDeduplication(jobType, data, deduplicationKey, customOptions = {}) {
