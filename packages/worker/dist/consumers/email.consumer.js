...
class EmailConsumer extends BaseConsumer {
        ...
        return this.withJobSpan(job, spanName, async () => {
            ...
            try {
                switch (type) {
                    ...
                    default:
                        throw new UnknownJobTypeError(WORKER_ERRORS.UNKNOWN_JOB_TYPE.replace("{type}", type), type);
                }
            }
            catch (error) {
    ...
    async handleVerificationEmail(job, data, jobLogger) {
        ...
        try {
            ...
            await this.emailService.sendVerificationEmail(data.user, data.token, data.locale || "en", { preferredProvider: data.preferredProvider } // Pass options
            );
