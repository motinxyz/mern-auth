...
export async function initializeCommonServices() {
    ...
    const results = await Promise.allSettled(services.map((s) => s.init().then(() => s.name)));
    ...
    if (failedServices.length > 0) {
        ...
        failedServices.forEach((failure) => {
            ...
            if (error instanceof DatabaseConnectionError) {
                logger.fatal({ err: error.originalError }, BOOTSTRAP_MESSAGES.DATABASE_CONNECTION_FAILED);
                criticalFailures.push(error);
            }
            else if (error instanceof RedisConnectionError) {
                logger.fatal({ err: error }, BOOTSTRAP_MESSAGES.REDIS_CONNECTION_FAILED);
                criticalFailures.push(error);
            }
            else {
                logger.error({ err: error, service: serviceName }, `${BOOTSTRAP_MESSAGES.SERVICE_START_ERROR}: ${serviceName}`);
...
export async function bootstrapApplication(app, onShutdown) {
    ...
    const server = app.listen(config.port, "0.0.0.0", () => {
        logger.info({ module: "bootstrap", port: config.port }, BOOTSTRAP_MESSAGES.SERVER_START_SUCCESS);
    });
    const gracefulShutdown = async (signal) => {
        logger.info({ signal }, `${BOOTSTRAP_MESSAGES.SHUTDOWN_SIGNAL_RECEIVED}: ${signal}`);
            ...
            server.close(async () => {
                ...
                if (onShutdown) {
                    try {
                        ...
                        logger.error({ err: error }, BOOTSTRAP_MESSAGES.CUSTOM_SHUTDOWN_ERROR);
                    }
                }
